...
# Keras/TensorFlow imports for GRU
...
class NSLKDDExcelProcessor:
    ...
    and GRU model training/evaluation.
    """
    ...
    def __init__(self):
        ...
        self.gru_model = None # Changed lstm_model to gru_model
    ...
    def build_gru_model(self, input_shape, num_classes):
        """
        Builds and compiles a GRU model for binary classification.

        Args:
            input_shape (tuple): Shape of the input data (timesteps, features).
                                 For a single time step per sample, it's (1, num_features).
        ...
        self.gru_model = model # Assign to gru_model
        print("GRU model built and compiled.")
        self.gru_model.summary()

    def train_gru_model(self, epochs=50, batch_size=32):
        """
        Trains the built GRU model.
        ...
        if self.gru_model is None:
            print("GRU model not built. Please call build_gru_model first.")
            return
        ...
        X_train_reshaped = self.X_train.to_numpy().reshape(self.X_train.shape[0], 1, self.X_train.shape[1])
        X_val_reshaped = self.X_val.to_numpy().reshape(self.X_val.shape[0], 1, self.X_val.shape[1])

        # Early stopping to prevent overfitting
        early_stopping = EarlyStopping(monitor='val_loss', patience=10, restore_best_weights=True)

        print("Training GRU model...")
        history = self.gru_model.fit( # Use gru_model
            X_train_reshaped,
            self.y_train,
            epochs=epochs,
            batch_size=batch_size,
            validation_data=(X_val_reshaped, self.y_val),
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig('gru_training_history.png') # Changed filename
        plt.show()

    def evaluate_gru_model(self):
        """
        Evaluates the trained GRU model on the test set.
        """
        if self.gru_model is None: # Use gru_model
            print("GRU model not trained. Please call train_gru_model first.")
            return
        ...
        X_test_reshaped = self.X_test.to_numpy().reshape(self.X_test.shape[0], 1, self.X_test.shape[1])

        print("\nEvaluating GRU model on test set...")
        loss, accuracy = self.gru_model.evaluate(X_test_reshaped, self.y_test, verbose=0) # Use gru_model
        print(f"Test Loss: {loss:.4f}")
        print(f"Test Accuracy: {accuracy:.4f}")

        y_pred_proba = self.gru_model.predict(X_test_reshaped) # Use gru_model
        ...
        plt.ylabel('Actual')
        plt.savefig('gru_confusion_matrix.png') # Changed filename
        plt.show()
        ...
        plt.savefig('gru_roc_curve.png') # Changed filename
        plt.show()
...
def process_nslkdd_excel_file(excel_path):
    ...
    and train/evaluate a GRU model.
    ...
    # Create train/test splits (important for GRU training)
    processor.create_train_test_splits(processed_df)

    # Build, train, and evaluate GRU model
    # The input_shape for GRU should be (timesteps, features)
    # Since each sample is a single measurement, timesteps = 1
    input_shape = (1, processor.X_train.shape[1])
    # num_classes should be 1 for binary classification with 'sigmoid' activation in the final Dense layer.
    num_classes = 1

    processor.build_gru_model(input_shape, num_classes=num_classes) # Changed to build_gru_model
    processor.train_gru_model(epochs=20, batch_size=64) # Changed to train_gru_model
    processor.evaluate_gru_model() # Changed to evaluate_gru_model
...
if __name__ == "__main__":
    ...
    print("5. GRU model trained and evaluated with performance plots.") # Changed message