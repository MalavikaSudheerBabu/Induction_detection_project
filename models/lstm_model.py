...
class NSLKDDExcelProcessor:
    ...
    def build_lstm_model(self, input_shape, num_classes):
        ...
        model = Sequential([
            ...
            Dropout(0.3),
            Dense(num_classes, activation='sigmoid') # For binary, use 'sigmoid' and 1 unit, or 'softmax' and 2 units
        ])

        # For binary classification, use binary_crossentropy and 1 unit in the last layer
        # For multi-class (if you switch to attack_category), use categorical_crossentropy
    ...
    def train_lstm_model(self, epochs=50, batch_size=32):
        ...
        if self.lstm_model is None:
            print("LSTM model not built. Please call build_lstm_model first.")
            return
    ...
    def evaluate_lstm_model(self):
        ...
        """
        if self.lstm_model is None:
            print("LSTM model not trained. Please call train_lstm_model first.")
            return
        ...
        loss, accuracy = self.lstm_model.evaluate(X_test_reshaped, self.y_test, verbose=0)
        print(f"Test Loss: {loss:.4f}")
        print(f"Test Accuracy: {accuracy:.4f}")

        y_pred_proba = self.lstm_model.predict(X_test_reshaped)
...
def process_nslkdd_excel_file(excel_path):
    ...
    and train/evaluate an LSTM model.
    ...
    # Create train/test splits (important for LSTM training)
    processor.create_train_test_splits(processed_df)

    # Build, train, and evaluate LSTM model
    # The input_shape for LSTM should be (timesteps, features)
    ...
    # If using 'softmax' and one-hot encoding for the target, it would be 2.
    num_classes = 1

    processor.build_lstm_model(input_shape, num_classes=num_classes)
    processor.train_lstm_model(epochs=20, batch_size=64) # Reduced epochs for faster execution
    processor.evaluate_lstm_model()
...
if __name__ == "__main__":
    ...
    print("5. LSTM model trained and evaluated with performance plots.")